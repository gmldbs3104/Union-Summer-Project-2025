...
## 📊API 명세서
| 기능 | 메서드 | URL | 요청 바디 (예시) | 요청 헤더 | 응답 (예시) |
|------|--------|-----|------------------|-----------|-------------|
| 서버 상태 확인 | GET | `/` | 없음 | 없음 | `<h1>WiFi Diagnosis System is Running</h1>` |
| 데이터 업로드 및 예측 | POST | `/upload` | ```json\n[\n  {\n    "sensor_mac": "00:11:22:33:44:55",\n    "timestamp": "2025-08-11 12:34:56",\n    "rssi": -67,\n    "ping": 23.5,\n    "speed": 45.6,\n    "ping_timeout": false\n  }\n]\n``` | `Content-Type: application/json` | ```json\n{\n  "status": "success",\n  "message": "총 1개의 데이터 처리 및 예측 완료."\n}\n``` |
| 최근 측정 데이터 조회 | GET | `/readings` | 없음 | 없음 | ```json\n{\n  "status": "success",\n  "data": [\n    {\n      "reading_id": 1,\n      "location": "서울 본사",\n      "ap_mac_address": "00:11:22:33:44:55",\n      "timestamp": "2025-08-11 12:34:56",\n      "rssi": -67,\n      "ping": 23.5,\n      "speed": 45.6,\n      "ping_timeout": false,\n      "speed_drop_rate": 0.1\n    }\n  ]\n}\n``` |
| 특정 측정 데이터 재예측 | POST | `/predict` | ```json\n{\n  "reading_id": 1\n}\n``` | `Content-Type: application/json` | ```json\n{\n  "status": "success",\n  "reading_id": 1,\n  "predicted_problem_type": "정상"\n}\n``` |

#### 변수
- sensor_mac: 센서 AP MAC 주소
- timestamp: 데이터 측정 시각 (미입력 시 현재 시각)
- rssi: 신호 세기 (dBm)
- ping: 지연시간 (ms)
- speed: 다운로드 속도 (Mbps)
- ping_timeout: Ping 타임아웃 여부 (boolean)
...
#### Slack 알림 규칙
- 예측 결과가 "정상" → 알림 미발송
- 예측 결과가 "트래픽 증가" → 경고(WARNING) 색상
- 그 외 모든 장애 → 오류(ERROR) 색상
...
#### 분류 규칙
- **DHCP/IP 충돌**: speed 측정 불가 또는 ping timeout.
- **공유기(AP) 문제**: ping ≥ 300ms & speed_drop_rate > 0.9.
- **통신사 백홀 문제**: 150ms ≤ ping < 300ms & 0.3 ≤ speed_drop_rate ≤ 0.5.
- **트래픽 증가**: ping > 이전 ping의 2배 & 0.2 ≤ speed_drop_rate ≤ 0.4.
- **전파 간섭**: RSSI < -70dBm & 50ms ≤ ping ≤ 100ms & speed_drop_rate > 0.4
<br>
