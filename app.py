...
def calculate_speed_drop_rates():
    ...
    try:
        ...
        # 1. 아직 speed_drop_rate가 NULL인 데이터만 가져오기
        cursor.execute("""
        ...
        """)
        null_speed_rates = cursor.fetchall()

        if not null_speed_rates:
            logging.info("계산할 데이터가 없어 속도 감소율을 계산할 내용이 없습니다.")
            return jsonify({"status": "success", "message": "계산할 데이터가 없습니다."})

        update_count = 0
        for row in null_speed_rates:
            try:
                # 2. 이전 레코드의 speed 값을 가져오기
                cursor.execute("""
                    SELECT speed
                    FROM f_sensor_readings
                ...
                if prev_speed_data:
                    # 3-1. 이전 데이터가 있으면 속도 감소율 계산
                    prev_speed = prev_speed_data['speed']
                    current_speed = row['speed']
                    
                    if prev_speed > 0:
                        speed_drop_rate = (prev_speed - current_speed) / prev_speed
                    else:
                        speed_drop_rate = 0.0
                else:
                    # 3-2. 이전 데이터가 없으면 (맨 처음 값) 속도 감소율을 0으로 설정
                    speed_drop_rate = 0.0
                ...
                cursor.execute("""
                    ...
                    WHERE reading_id = %s
                """, (speed_drop_rate, row["reading_id"]))
                update_count += 1
